...
class DeblurMask(nn.Module):
    def __init__(self, in_channels, kernel_size=3, n_feat=16):
        ...
        self.head = nn.Sequential(
            nn.Conv2d(in_channels * 1, n_feat, kernel_size=kernel_size, padding=kernel_size // 2, stride=1)
        )
    ...
    def forward(self, x, psf):
        ...
        # out = t.cat([], dim=1)
        out = self.robust(t.abs(x - reb), 0.05)
        # out = t.cat([t.abs(x - reb), 0.05], dim=1)
        a = self.head(out)
        a1 = self.body1(a)
        a2 = self.body2(a1)
        a3 = self.body3(a2)
...
class DeblurMaskED(nn.Module):
    ...
    def forward(self, blur, reblur):

        out = self.robust(t.abs(blur - reblur), 0.039)
        # c = out.shape[1]
        # out, _ = t.min(out, dim=1, keepdim=True)
        # out = out.repeat(1, 3, 1, 1)
        # out = t.where(out < 0.6, t.zeros_like(out), t.ones_like(out))
