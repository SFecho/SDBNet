...
from model import ModelManager, make_model
...
class TestModel(ProjectTestCase):
    ...
    def setup_method(self, method):
        ...
        self.net = make_model(self.cfg.network)
        self.loss_f = nn.CrossEntropyLoss()
        self.model = ModelManager(self.cfg, self.net, self.loss_f)

    def test_model(self):
        assert self.model.cfg == self.cfg
        assert self.model.net == self.net
        assert self.model.loss == self.loss_f
    ...
    def test_save_load_network(self):
        ...
        local_model = ModelManager(self.cfg, local_net, self.loss_f)

        self.model.save_network()
        save_filename = "%s_%d.pt" % (self.cfg.name, self.model.batch_step)
    ...
    def test_save_load_state(self):
        ...
        save_filename = "%s_%d.state" % (self.cfg.name, self.model.batch_step)
        save_path = os.path.join(self.cfg.log.chkpt_dir, save_filename)
        ...
        assert local_model.batch_step == self.model.batch_step
